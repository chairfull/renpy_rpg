...
default player = None # Main player character.
...
init -90 python:
    ...
    class Being(HasScreen, HasZone, HasTraits, HasFlags, HasItems, PathFollower):
        def __init__(self, _id, name, desc, **kwargs):
            HasScreen.__init__(self, **kwargs)
            HasZone.__init__(self, **kwargs)
            HasTraits.__init__(self, **kwargs)
            HasFlags.__init__(self, **kwargs)
            HasItems.__init__(self, **kwargs)
        ...
        def cache_stats(self):
            ...
            for clan in self.get_traits(Faction):
        ...
        @property
        def is_male(self): return self.has_trait("gender", "m")

        @property
        def is_female(self): return self.has_trait("gender", "f")
        ...
        def set_zone(self, zone):
            ...
            if super().set_zone(zone):
                ZONE_EXITED.emit(zone=last_zone, being=self)
                ZONE_ENTERED.emit(zone=self.zone, being=self)
    ...
    PLAYER_CHANGED = create_signal()

    @flow_action("PLAYER")
    def set_player(self, being):
        global player
        player = being
        PLAYER_CHANGED.emit()
