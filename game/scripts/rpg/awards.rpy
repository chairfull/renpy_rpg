...
init 10 python:
    onstart(add_meta_menu_tab, "awards", "🏆", "Achievements",
        selected_award=None)

    class Award(HasTags):
        def __init__(self, id, name, desc, icon="🏆", tags=None, trigger=None, max_ticks=1, **kwargs):
        ...
        def unlock(self):
            if self.tick < self.max_ticks:
                ...
                AWARD_UNLOCKED.emit(award=self)
    
    AWARD_UNLOCKED = create_signal(award=award)
    
    @flow_action("AWARD")
    def unlock_award(id):
        award = all_awards.get(id)
        if award:
            award.unlock()
